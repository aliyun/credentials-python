...
EXECUTOR = _DaemonThreadPoolExecutor(max_workers=None)
...
def _max_stale_failure_jitter(num_failures: int) -> int:
    backoff_millis = min(10 * 1000, (1 << max(0, num_failures - 1)) * 100)
...
class RefreshCachedSupplier(Generic[T]):
    ...
    def _handle_fetched_failure(self, exception: Exception) -> RefreshResult[T]:
        ...
        if self._stale_value_behavior == StaleValueBehavior.STRICT:
            ...
            new_stale_time = _jitter_time(now, 1,
                                          int(_max_stale_failure_jitter(self._consecutive_refresh_failures) / 1000))
            log.warning(
