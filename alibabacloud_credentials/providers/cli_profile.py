...
class CLIProfileCredentialsProvider(ICredentialsProvider):
    ...
    def __init__(self, *,
        ...
        self.__profile_file = os.path.join(ac.HOME, "/.aliyun/config.json")
    ...
    def get_credentials(self) -> Credentials:
        if au.environment_cli_profile_disabled:
            raise CredentialException('cli credentials file is disabled')

        if self._should_reload_credentials_provider():
            if not os.path.exists(self.__profile_file) or not os.path.isfile(self.__profile_file):
                raise CredentialException(f'unable to open credentials file: {self.__profile_file}')
            try:
                config = _load_config(self.__profile_file)
            except Exception as e:
                raise CredentialException(
                    f'failed to parse credential form cli credentials file: {self.__profile_file}')
            if config is None:
                raise CredentialException(
                    f'failed to parse credential form cli credentials file: {self.__profile_file}')
    ...
    async def get_credentials_async(self) -> Credentials:
        ...
        if self._should_reload_credentials_provider():
            if not os.path.exists(self.__profile_file) or not os.path.isfile(self.__profile_file):
                raise CredentialException(f'unable to open credentials file: {self.__profile_file}')
            try:
                config = await _load_config_async(self.__profile_file)
            except Exception as e:
                raise CredentialException(
                    f'failed to parse credential form cli credentials file: {self.__profile_file}')
            if config is None:
