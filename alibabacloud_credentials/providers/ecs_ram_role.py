...
from .refreshable import Credentials, RefreshResult, StaleValueBehavior, RefreshCachedSupplier, NonBlocking
from . import HttpOptions
    ...
    def __init__(self, *,
        ...
        if async_update_enabled:
            self._credentials_cache = RefreshCachedSupplier(
                refresh_callable=self._create_credential,
                refresh_callable_async=self._create_credential_async,
            ...
            async def refresh_task():
                if self._should_refresh:
                    log.debug(f'Executing refresh attempt scheduled for {background_refresh_time}')
